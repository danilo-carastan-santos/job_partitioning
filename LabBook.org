...
** [2021-11-17 mer.] Datamove-TT presentation
Questions and discussions:

- When do you decide to partition a job?
  Directly at the arrival/start of ajob or when the platform becomes clogged?

  R: At arrival time, in general. But, it can be a good idea to just partition when the platform
     becomes clogged.

- If we make the partitions at the arrival time, then all the partitions will be close in the queue?

  R: No, the partitions will be sent to some place else, and will be got back when a previous partition
  finishes. Maybe?

- Do we really a partition? Why not to just delay?

  - We can compare the difference between delaying and partitioning.
  - If we delay, how long we should delay?
  R: One good question here is that it solves partialy the waiting time of the small
     jobs and the long. If you delay 100%, the large jobs we will have big waiting time, and
     maybe we don't want that.

- Did you try submitting the next part of the partitioned job when at 80% or 50% of the previous part ? (instead of 100% each time)

  R: It can be good for reducing the waiting time of the next partitions, but then we need to
     be carefull with new constraints, such as, not starting a next partition before the previous
     one is finished.
